...
enum RefreshType { listRecentData, listPOData }
...
class InVM extends GetxController {
  ...
  final RxList<InModel> tolistPORecent = <InModel>[].obs;
  ...
  final RxList<InModel> filteredPOList = <InModel>[].obs;
  ...
  void searchPOData(String query) {
    if (query.isEmpty) {
      clearFilters();
      return;
    }
    ...
    // Gunakan tolistPObackup atau tolistPO sebagai sumber data
    final sourceList = tolistPObackup.isNotEmpty ? tolistPObackup : tolistPO;

    final filteredList = sourceList.where((po) {
      final documentNo = po.documentno?.toLowerCase() ?? '';
      final vendor = po.cBpartnerId?.toLowerCase() ?? '';
      final vendorPO = po.vendorpo?.toLowerCase() ?? '';

      return documentNo.contains(searchQuery) ||
          vendor.contains(searchQuery) ||
          vendorPO.contains(searchQuery);
    }).toList();

    filteredPOList.assignAll(filteredList);
  }

  void sortPOData(String sortBy) {
    List<InModel> listToSort = List.from(
      filteredPOList.isNotEmpty ? filteredPOList : tolistPO,
    );
    ...
    switch (sortBy) {
      case 'PO Date':
        listToSort.sort((a, b) {
          ...
          return bDate.compareTo(aDate); // Descending (terbaru dulu)
        });
        break;
      case 'Vendor':
        listToSort.sort((a, b) {
          final aVendor = a.cBpartnerId ?? '';
          final bVendor = b.cBpartnerId ?? '';
          return aVendor.compareTo(bVendor); // Ascending (A-Z)
        });
        break;
      case 'All':
      default:
        // Tidak melakukan sorting, tampilkan data asli
        listToSort = List.from(tolistPO);
        break;
    }
  ...
  void clearFilters() {
    ...
    filteredPOList.assignAll(tolistPO);
  ...
  Future<void> refreshDetailsData(String documentNo) async {
    try {
      ...
      final List<InDetail> refreshedDetails = await getDetailsByDocumentNo(
        documentNo,
      );
  ...
  Future<bool> approveIn(
    ...
    try {
      ...
      await FirebaseFirestore.instance
          .collection('in')
          .doc(inmodel.documentno)
          .set({
            'dateordered': inmodel.dateordered,
            'documentno': inmodel.documentno,
            'docstatus': inmodel.docstatus,
            'ad_client_id': inmodel.adClientId,
            'ad_org_id': inmodel.adOrgId,
            'c_bpartner_id': inmodel.cBpartnerId,
            'c_currency_id': inmodel.cCurrencyId,
            'c_doctype_id': inmodel.cDoctypeId,
            'c_doctypetarget_id': inmodel.cDoctypetargetId,
            'deliveryviarule': inmodel.deliveryviarule,
            'details': tdata,
            'freightcostrule': inmodel.freightcostrule,
            'm_pricelist_id': inmodel.mPricelistId,
            'm_product_category_id': inmodel.mProductCategoryId,
            'm_warehouse_id': inmodel.mWarehouseId,
            'priorityrule': inmodel.priorityrule,
            'totallines': inmodel.totallines,
            'user1_id': inmodel.user1Id,
  ...
  Future<List<InModel>> getData(String documentno, String category) async {
    try {
      final query = await FirebaseFirestore.instance
          .collection('in')
  ...
  Stream<List<InModel>> listPO() {
    try {
      ...
      _logger.e('Error in listPO: $e');
      return Stream.value(<InModel>[]);
    }
  }
  ...
  Stream<List<InModel>> listforRecentALL() {
    try {
      ...
      return FirebaseFirestore.instance
          ...
          .limit(10)
          .snapshots()
          .map<List<InModel>>((query) {
            final outmodellocal = <InModel>[];

            for (final doc in query.docs) {
              try {
                final returnpo = InModel.fromDocumentSnapshot(doc);
            ...
            tolistPORecent.assignAll(outmodellocal);
  ...
  Future<void> refreshData({RefreshType type = RefreshType.listPOData}) async {
    ...
    try {
      switch (type) {
        case RefreshType.listRecentData:
          srlist.bindStream(listforRecentALL());
          break;
        case RefreshType.listPOData:
          srlist.bindStream(listPO());
          break;
      }
  ...
  Future<List<InDetail>> getDetailsByDocumentNo(String documentNo) async {
    try {
      ...
      final inModel = InModel.fromDocumentSnapshot(doc);

      _logger.i('Berhasil mengambil data InModel: ${inModel.documentno}');

      if (inModel.details == null || inModel.details!.isEmpty) {
  ...
  Stream<List<InDetail>> getDetailsByDocumentNoWithFilter(String documentNo) {
    try {
      _logger.i(
      ...
      return Stream.value(<InDetail>[]);
    }
  }

  Stream<InModel?> getPODataStream(String documentNo) {
    try {
      ...
      return FirebaseFirestore.instance
          ...
          .map((querySnapshot) {
            ...
            _logger.i('Berhasil update PO data: ${inModel.documentno}');
            return inModel;
          })
          .handleError((error) {
