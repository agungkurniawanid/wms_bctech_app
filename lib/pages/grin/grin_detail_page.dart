...
class GrinDetailPage extends StatefulWidget {
  final String grId;

  const GrinDetailPage({super.key, required this.grId});

  @override
  State<GrinDetailPage> createState() => _GrinDetailPageState();
}

class _GrinDetailPageState extends State<GrinDetailPage> {
  ...
  GoodReceiveSerialNumberModel? _grinData;
  ...
  List<Map<String, dynamic>> _processDetailItems(
    List<GoodReceiveSerialNumberDetailModel> details,
  ) {
  ...
  Future<void> _saveQuantity() async {
    ...
    try {
      // Update di Firestore
      final grinRef = _firestore.collection('gr_in').doc(widget.grId);
